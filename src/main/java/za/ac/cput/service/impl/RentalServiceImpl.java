    ...
    public Rental create(Rental rental) {
        if (isBookAvailable(rental)) {
            if (isCurrentlyRenting(rental.getUser())) {
                throw new UserCantRentMoreThanOneBookException(rental.getUser().getFirstName() + " " + rental.getUser().getLastName() + " is currently renting " + getCurrentRental(rental.getUser()).getBook().getTitle() + " " + getCurrentRental(rental.getUser()).getBook().getAuthor() + " " + getCurrentRental(rental.getUser()).getBook().getDescription());
            }
            Rental newRental = rentalFactory.create(rental);
            return repository.save(newRental);
        } else {
            throw new BookNotAvailableException(rental.getBook() + ", is not available for rental at this time");
        }
    }
